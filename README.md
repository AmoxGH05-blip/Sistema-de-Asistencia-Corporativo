...
#### ¿Tu contraseña de MySQL NO es `root`?

Abre el archivo `app.py` con cualquier editor de texto (clic derecho → Abrir con → Bloc de notas).  
Busca estas líneas cerca de la línea 22:

```python
db_config = {
    'host': 'localhost',
    'user': 'root',
    'password': 'root',      # <-- Cambia 'root' por tu contraseña real
...
| `Access denied for user 'root'@'localhost'` | La contraseña de MySQL es incorrecta | Abre `app.py` y cambia el valor de `'password'` en `db_config` |
...
- La contraseña administrativa para dar de baja empleados es `Abd6S` — cámbiala en `app.py` si lo necesitas
